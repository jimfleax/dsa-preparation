...
export default function UntrackedProblemsModal({ isOpen, onClose, onTracked }: UntrackedProblemsModalProps) {
  ...
  const handleToggleTrack = async (id: string) => {
    ...
    try {
      const token = await getToken();
      const response = await fetch(`${apiBase}/api/problems/${id}/toggle-track`, {
        method: "PATCH",
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await response.json();
        ...
        <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl border border-neutral-100 overflow-hidden animate-in fade-in zoom-in-95 duration-200 max-h-[90dvh] flex flex-col">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-neutral-100 shrink-0">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h2 className="text-sm font-bold text-neutral-900">Untracked Problems</h2>
                <p className="text-[11px] text-neutral-400 font-medium">Problems you dismissed from sync</p>
              </div>
            </div>
            <button onClick={onClose} className="p-2 hover:bg-neutral-50 rounded-lg text-neutral-400 hover:text-neutral-700 transition-all cursor-pointer">
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-2">
            {loading ? (
              <div className="py-12 flex flex-col items-center justify-center text-neutral-400">
                ...
                <span className="text-xs font-medium">Loading untracked problems...</span>
              </div>
            ) : problems.length === 0 ? (
              <div className="py-12 flex flex-col items-center justify-center text-center">
                ...
                <h3 className="text-sm font-bold text-neutral-700">No Untracked Problems</h3>
                <p className="text-xs text-neutral-500 mt-1">All your LeetCode submissions are being tracked.</p>
              </div>
            ) : (
              <div className="space-y-1">
                {problems.map(problem => (
                  <div key={problem._id} className="flex items-center justify-between p-3 hover:bg-neutral-50 rounded-xl transition-colors border border-transparent hover:border-neutral-100">
                    <div className="flex items-center gap-3">
                      <span className="text-sm font-semibold text-neutral-800">{problem.title}</span>
                      {problem.difficulty && (
                    ...
                    >
                      {togglingId === problem._id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <RefreshCcw className="w-3.5 h-3.5" />}
                      Track
                    </button>
                  </div>
                ))}
          ...
          <div className="px-6 py-3 border-t border-neutral-100 bg-neutral-50 shrink-0 flex justify-between items-center text-[11px] font-medium text-neutral-500">
            <span>Showing {problems.length} untracked problem{problems.length !== 1 ? 's' : ''}</span>
            <button onClick={onClose} className="px-3 py-1.5 bg-white text-neutral-700 rounded-lg border border-neutral-200 font-bold hover:bg-neutral-50">Close</button>
          </div>
        </div>
      </div>
    </>
