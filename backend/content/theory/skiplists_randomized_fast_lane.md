...
- **Level 0 (The Base):** A standard, single-step sorted Linked List containing *all* elements.
...
2. **Scan Right:** Is the *next* node on this level less than or equal to `X`? If yes, move right.
3. **Drop Down:** If the next node is *greater* than `X` (you would overshoot), or if it's `Null`, you drop down one level on your current node.
...
*Every node's height is determined by how many consecutive Heads it flipped upon insertion.*
...
*\*The $O(N)$ worst-case for a SkipList occurs only if you flip "Tails" on every single insertion, leaving you with just Level 0. The probability of this happening on a large dataset is infinitesimally small (essentially zero).*
