...
async function runTests() {
  ...
  try {
    ...
    await requireAdminAuth(
      mockReqMiddleware as any,
      mockResLogin as any,
      mockNext as any,
    );
    if (!nextCalled || mockReqMiddleware.admin?.id !== admin.id)
      throw new Error("Middleware failed");
    console.log(
      "Middleware Success! Admin ID attached:",
      mockReqMiddleware.admin.id,
    );
    ...
    if (
      analyticsData.users.total !== 2 ||
      analyticsData.users.newLast30Days !== 1
    )
      throw new Error("Users metrics wrong");
    if (
      analyticsData.content.totalTracks !== 2 ||
      analyticsData.content.totalProblemsAvailable !== 3
    )
      throw new Error("Content metrics wrong");
    if (analyticsData.engagement.totalProblemsSolvedGlobally !== 3)
      throw new Error("Global solved wrong");
    if (
      analyticsData.completionRate.solved !== 2 ||
      analyticsData.completionRate.revising !== 1 ||
      analyticsData.completionRate.unsolved !== 3
    )
      throw new Error("Completion rate wrong");
