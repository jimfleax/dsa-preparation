...
const withDueFlag = (doc: any) => {
  ...
  return {
    ...obj,
    isDueToday: calculateIsDue(obj.lastAttemptedDate, obj.reviewDurationDays),
  };
};
 ...
 * Body: { url: string }
 */
export const scrapeLeetCodeTitle = catchAsync(
 ...
 */
export const getLeetCodeCalendar = catchAsync(
  async (req: Request, res: Response) => {
 ...
 */
export const revisitProblem = catchAsync(
  async (req: Request, res: Response) => {
    const userId = req.user?.id;
    if (!userId) {
      throw AppError.unauthorized("Unauthorized");
 ...
 */
export const revisitProblemBySlug = catchAsync(
  async (req: Request, res: Response) => {
    const userId = req.user?.id;
    if (!userId) {
      throw AppError.unauthorized("Unauthorized");
 ...
 */
export const listUntrackedProblems = catchAsync(
  async (req: Request, res: Response) => {
    const userId = req.user?.id;
    if (!userId) {
      throw AppError.unauthorized("Unauthorized");
 ...
 */
export const toggleTrackProblem = catchAsync(
  async (req: Request, res: Response) => {
    const userId = req.user?.id;
    if (!userId) {
      throw AppError.unauthorized("Unauthorized");
 ...
 */
export const getSolvedSlugs = catchAsync(
  async (req: Request, res: Response) => {
    const userId = req.user?.id;
    if (!userId) {
      throw AppError.unauthorized("Unauthorized");
 ...
 */
export const getDueProblems = catchAsync(
  async (req: Request, res: Response) => {
    const userId = req.user?.id;
    if (!userId) {
      throw AppError.unauthorized("Unauthorized");
    ...
    const problems = await TrackedProblem.find({
      ...
      .select(
        "titleSlug title difficulty lastAttemptedDate reviewDurationDays attemptCount url",
      )
      .lean();
    const dueProblems = problems.filter((p: any) =>
      calculateIsDue(p.lastAttemptedDate, p.reviewDurationDays),
    );
    const dueProblemsWithFlag = dueProblems.map((p: any) => withDueFlag(p));
    res.json({
      success: true,
      problems: dueProblemsWithFlag,
      count: dueProblemsWithFlag.length,
    });
  },
);
 ...
 */
export const getSlimProblems = catchAsync(
  async (req: Request, res: Response) => {
    const userId = req.user?.id;
    if (!userId) {
      throw AppError.unauthorized("Unauthorized");
 ...
 */
export const getTrackerMetrics = catchAsync(
  async (req: Request, res: Response) => {
    const userId = req.user?.id;
    if (!userId) {
      throw AppError.unauthorized("Unauthorized");
