...
flashing yellow. The *specific* steps of the cycle are flexible. But that one rule? Absolute.
...
The invariant is what separates "code that seems to work" from "code you can *prove* works."
...
But more precisely: it's not that the *value* never changes. It's that a certain *property*
or *relationship* always remains true — regardless of what step you're on, what iteration
...
Here's the honest answer: computers are deterministic but *humans are fallible*. When you
write a loop that runs 10,000 times, you can't manually verify every single iteration is
correct. You need a *proof technique* — a way to reason about all iterations at once,
...
*process* — because every individual transaction was designed to preserve the rule. The
**invariant is the process's promise to itself.**

When an accountant finds A ≠ L + E, they know exactly one thing: *somewhere, a transaction
broke the rule.* They don't need to check everything — the invariant points to the bug.

This is *exactly* what happens in algorithms. When your binary search returns a wrong index,
...
This is *literally* mathematical induction dressed up as a loop proof:
...
This governs the *internal rules* of a data structure. It says: "For this data structure to
...
Every method in the data structure has a *contract*: "I receive the structure in a valid state,
...
invariant that makes it *provably correct*.
...
**The Invariant:**  
> *"If target `t` exists in the array, it is always located within `arr[low..high]` (inclusive)."*
...
within `arr[low..high]`." But `low > high` means that range is *empty*. Therefore, `t` doesn't
...
still inside our search space `arr[low..high]`. But we already *know* `arr[mid] ≠ target`!
...
The invariant was preserved *in letter* but violated *in spirit* — we stopped making progress.
...
This *progress condition* is also part of the invariant. Violating it isn't a logic error — it's
...
At the start, the invariant is a **glowing fence** that encloses the *entire* hallway. Everything
...
The invariant is the fence. It always encloses a region where the target *could* be. The fence
never stops being true — it just gets smaller and smaller. When the fence encloses *zero* doors,
...
The fence is the invariant made *visible*.
...
3. **"If I show you a binary search implementation that has an infinite loop on some inputs, what
   ...
   *(Hint: it's not the correctness invariant — it's the other one.)*
...
The mistake is treating the invariant as *documentation* rather than as a *proof obligation*.
The invariant should actively *drive* the design of your loop. When you're deciding "do I use
...
*This* is checkable, precise, and at termination (`k = n-1`), it proves the entire array is sorted.
...
The postcondition *derives from* the invariant + exit condition. Think of it this way:
...
The invariant is the *cause*. The postcondition is the *effect*. Don't confuse them.
...
`low = mid + 1` caused infinite loops or off-by-one errors? *That was entirely an invariant
story.* Each convention defines a slightly different invariant about what the search space
...
guarantee the search space shrinks" — *is* the progress invariant, stated concretely.
...
That guarantee *is* the algorithm invariant of the partition step:
> "After `partition(arr, low, high)` returns index `p`, `arr[p]` is in its correct final
> position, `arr[low..p-1] ≤ arr[p] ≤ arr[p+1..high]`."

Quicksort works *because* this invariant holds after every call to `partition`. Each recursive
...
*"An invariant is a promise your algorithm makes to itself — on every step, at every scale.
Understand the promise, and you understand the algorithm."*