...
```typescript
  ...
  await Track.create({ title: "Test Track", description: "Testing", order: 1, problems: [{ title: "Two Sum", titleSlug: "two-sum", difficulty: "Easy", url: "http://example.com/1" }] });
  // Create 2 TrackedProblems (two-sum and three-sum)
  await TrackedProblem.create({ userId: "user_1", titleSlug: "two-sum", title: "Two Sum", attemptCount: 1 });
  await TrackedProblem.create({ userId: "user_1", titleSlug: "three-sum", title: "Three Sum", attemptCount: 1 });
  // ... run getAnalytics ...
```
**Output:**
```json
