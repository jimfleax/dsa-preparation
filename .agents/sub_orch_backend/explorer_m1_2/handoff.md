...
3. **Analytics Logic:**
   ...
   - **Completion Rate:**
     - _Revising:_ `TrackedProblem` records with `notrack: { $ne: true }` and `reviewDurationDays: { $exists: true, $ne: null }`.
     - _Solved (without revision):_ Total Solved - Revising.
     - _Unsolved:_ `(Total Users * Total Unique Problems) - Total Solved`.
