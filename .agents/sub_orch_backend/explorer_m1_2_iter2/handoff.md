...
- At lines 29-37, `uniqueProblems` is computed _after_ fetching the `tracks` to calculate `totalProblemsAvailable`.
...
The implementation of `getAnalytics` is mathematically flawed due to unbounded `TrackedProblem` counts for curriculum metrics. The fix is to extract `solvedCount` and `revisingCount` from the initial `Promise.all` and fetch them sequentially _after_ computing the array of `uniqueProblems`.
