...
export default function ReviewDuePopup({ problems, onRevisited }: ReviewDuePopupProps) {
  ...
  const handleRevisit = async (problemId: string, reviewDurationDays: number | null) => {
    setRevisitingId(problemId);
    try {
      const token = await getToken();
      const response = await fetch(`${apiBase}/api/tracker/${problemId}/revisit`, {
        method: "PATCH",
        headers: {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-4 w-full max-w-sm">
      {dueProblems.map((problem) => (
        <div
          key={problem._id}
          className="bg-white rounded-2xl shadow-2xl border border-indigo-100 overflow-hidden animate-in slide-in-from-bottom-5 fade-in duration-300"
        >
          <div className="flex items-center justify-between px-4 py-3 bg-indigo-50 border-b border-indigo-100">
            ...
            <button
              onClick={() => handleDismiss(problem._id)}
          ...
          <div className="p-4 space-y-4">
            ...
            {showKeepReviewingId === problem._id ? (
              <div className="space-y-3 animate-in fade-in zoom-in-95 duration-200">
                <div className="flex items-center gap-2">
                  <input
                    ...
                    min="1"
                    placeholder="Days"
                    value={keepReviewDuration[problem._id] || ""}
                    onChange={(e) =>
                      setKeepReviewDuration({ ...keepReviewDuration, [problem._id]: e.target.value })
                    }
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setShowKeepReviewingId(null)}
                  ...
                  <button
                    onClick={() =>
                      handleRevisit(
                        problem._id,
                        parseInt(keepReviewDuration[problem._id]) || problem.reviewDurationDays || 1
                      )
                    }
                    disabled={revisitingId === problem._id || !keepReviewDuration[problem._id]}
              ...
              <div className="flex flex-col gap-2">
                <button
                  onClick={() => handleRevisit(problem._id, null)}
                  disabled={revisitingId === problem._id}
                ...
                <button
                  onClick={() => setShowKeepReviewingId(problem._id)}
                  disabled={revisitingId === problem._id}
