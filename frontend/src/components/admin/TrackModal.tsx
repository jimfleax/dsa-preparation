...
export default function TrackModal({
  ...
  // Inner modal states
  const [addProblemTarget, setAddProblemTarget] = useState<
    number | "root" | null
  >(null);
  ...
  const [editSubtrackTarget, setEditSubtrackTarget] = useState<number | null>(
    null,
  );
  const [editProblemTarget, setEditProblemTarget] = useState<{
    partIndex: number | "root";
    problemIndex: number;
  } | null>(null);
  ...
  const handleEditProblemClick = (
    ...
    const prob =
      partIndex === "root"
        ...
        : parts[partIndex].problems[problemIndex];
  ...
  const renderProblemCard = (
    ...
    <div
      key={pIdx}
      className="flex items-center justify-between p-3 bg-white border border-neutral-100 rounded-xl shadow-sm mb-2 group"
    >
      ...
      <div className="relative overflow-hidden min-h-[450px]">
        ...
        >
          ...
          <div className="absolute bottom-0 left-0 w-full p-6 bg-white border-t border-neutral-100 flex justify-end gap-3">
            <button
              onClick={onClose}
              className="px-5 py-2.5 text-sm font-bold text-neutral-600 hover:text-neutral-900 border border-neutral-200 rounded-xl hover:bg-neutral-50 transition-all"
            >
              Cancel
            </button>
            <button
              onClick={handleNextPhase}
              className="px-5 py-2.5 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl shadow-sm hover:shadow-md flex items-center gap-2 transition-all"
            >
        ...
        {/* Phase 2: Structure */}
        <div
          className={`absolute inset-0 p-6 flex flex-col transition-transform duration-300 ${phase === 2 ? "translate-x-0" : "translate-x-full"}`}
        >
          <div className="flex-1 overflow-y-auto pr-2 pb-24">
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setAddSubtrackOpen(true)}
                ...
                <button
                  onClick={() => setAddProblemTarget("root")}
                  className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-lg transition-colors"
                >
              ...
              !addSubtrackOpen && (
                <div className="text-center p-8 bg-neutral-50 border border-neutral-100 rounded-2xl border-dashed">
                  <p className="text-sm text-neutral-500 font-medium">
                    No items added yet. Add a subtrack or an individual problem
                    to get started.
                  </p>
                </div>
              )}
            ...
            {problems.length > 0 && (
              <div className="mb-6">
                <h4 className="text-xs font-bold text-neutral-500 uppercase tracking-wider mb-3 pl-1">
                  Root Problems
                </h4>
            ...
            {parts.map((part, i) => (
              <div
                key={i}
                className="mb-6 bg-neutral-50 border border-neutral-200 rounded-2xl p-4"
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => handleEditSubtrackClick(i)}
                      className="flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-bold text-neutral-600 hover:text-indigo-600 hover:bg-indigo-50 rounded-md transition-colors"
                    >
                      <Edit2 className="w-3 h-3" /> Edit
                    </button>
                    <button
                      onClick={() => setAddProblemTarget(i)}
                      className="flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-bold text-indigo-600 hover:bg-indigo-100 rounded-md transition-colors"
                    >
                      <Plus className="w-3 h-3" /> Add Problem
                    </button>
                    <button
                      onClick={() => removeSubtrack(i)}
                ...
                <div>
                  {part.problems.length === 0 ? (
                    <p className="text-xs text-neutral-400 italic pl-1">
                      No problems in this subtrack.
                    </p>
                  ) : (
                    part.problems.map((p: any, pIdx: number) =>
                      renderProblemCard(p, pIdx, i),
                    )
                  )}
                </div>
              </div>
          ...
          <div className="absolute bottom-0 left-0 w-full p-6 bg-white border-t border-neutral-100 flex justify-between gap-3">
            <button
              onClick={() => setPhase(1)}
              className="px-5 py-2.5 text-sm font-bold text-neutral-600 hover:text-neutral-900 border border-neutral-200 rounded-xl hover:bg-neutral-50 flex items-center gap-2 transition-all"
            >
              ...
              {addSubtrackOpen ? (
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <input
                        autoFocus
                        value={subtrackTitle}
                        onChange={(e) => setSubtrackTitle(e.target.value)}
                        placeholder="e.g. Two Pointers"
                        className="w-full px-3 py-2 bg-neutral-50 border border-neutral-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-indigo-500/20 outline-none"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-neutral-700 mb-1">
                      ...
                      <textarea
                        value={subtrackDescription}
                        onChange={(e) => setSubtrackDescription(e.target.value)}
                        className="w-full px-3 py-2 bg-neutral-50 border border-neutral-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-indigo-500/20 outline-none resize-none h-20"
                      />
                    </div>
                  </div>
                ...
                <>
                  ...
                  <div className="space-y-4">
                    ...
                    <div
                      className={`w-full px-4 py-2.5 bg-neutral-50 border border-neutral-100 rounded-xl text-sm font-medium min-h-[42px] flex items-center justify-between ${problemPreview ? "text-neutral-800" : "text-neutral-400 italic"}`}
                    >
                      <div className="flex items-center gap-3">
                        <span>
                          {problemPreview?.title ||
                            "Enter URL to fetch title..."}
                        </span>
                        {problemPreview?.difficulty && (
                          <span
                            className={`px-2 py-0.5 rounded-full text-[10px] font-bold tracking-wide uppercase border ${
                              problemPreview.difficulty === "Easy"
                                ? "bg-emerald-50 text-emerald-700 border-emerald-100/50"
                                : problemPreview.difficulty === "Medium"
                                  ? "bg-amber-50 text-amber-700 border-amber-100/50"
                                  : "bg-rose-50 text-rose-700 border-rose-100/50"
                            }`}
                          >
                  ...
                  <div className="flex justify-end gap-2 mt-6">
                    ...
                    <button
                      ...
                      disabled={!problemPreview || isFetchingProblem}
                      className="px-4 py-2 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg disabled:opacity-50 transition-colors"
                    >
                      {editProblemTarget !== null
                        ? "Save Problem"
