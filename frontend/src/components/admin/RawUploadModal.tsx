...
export default function RawUploadModal({
  ...
  const handleHandoff = () => {
    ...
    const finalParts = parseResult.parts
      .map((part) => ({
        title: part.title,
      ...
      .filter(
        (part) => part.problems.length > 0 || parseResult.parts.length > 0,
      );
  ...
  return (
    <BaseModal
      ...
      subtitle={
        <Stepper
          steps={["Copy Instructions", "Paste Response", "Fetch from LeetCode"]}
          currentStep={phase}
        />
      }
    ...
    >
      <div className="relative overflow-hidden min-h-[500px]">
        ...
        <div
          className={`absolute inset-0 p-6 flex flex-col transition-transform duration-300 ${phase === 1 ? "translate-x-0" : "-translate-x-full"}`}
        >
          <div className="flex-1 overflow-y-auto">
            <p className="text-sm text-neutral-600 mb-4">
              To bulk-import a track, you can use any AI assistant (ChatGPT,
              Claude, etc.) to generate the required LeetCode URLs. Copy the
              instructions below, paste them into your AI, and copy its
              response.
            </p>
        ...
        <div
          className={`absolute inset-0 p-6 flex flex-col transition-transform duration-300 ${phase === 2 ? "translate-x-0" : phase < 2 ? "translate-x-full" : "-translate-x-full"}`}
        >
              ...
              <div className="mt-4 p-4 bg-rose-50 border border-rose-200 rounded-xl shrink-0 max-h-40 overflow-y-auto">
                ...
                <ul className="space-y-1">
                  {parseResult.errors.map((err, i) => (
                    ...
                    >
                      <span className="font-mono text-rose-400 bg-rose-100 px-1.5 py-0.5 rounded shrink-0">
                        Line {err.line}
                      </span>
              ...
              <div className="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-xl shrink-0 max-h-32 overflow-y-auto">
                ...
                <ul className="space-y-1">
                  {parseResult.warnings.map((warn, i) => (
                    ...
                    >
                      <span className="font-mono text-amber-400 bg-amber-100 px-1.5 py-0.5 rounded shrink-0">
                        Line {warn.line}
                      </span>
        ...
        {/* Phase 3: Fetching */}
        <div
          className={`absolute inset-0 p-6 flex flex-col transition-transform duration-300 ${phase === 3 ? "translate-x-0" : "translate-x-full"}`}
        >
          <div className="flex-1 overflow-y-auto">
            {parseResult && (
              <>
                ...
                <div className="bg-neutral-50 border border-neutral-200 rounded-2xl p-6 mb-6">
                  <div className="flex justify-between items-end mb-2">
                    ...
                    <span className="text-xs font-bold text-neutral-500">
                      {fetchedProblems.length + failedUrls.length} /{" "}
                      {parseResult.totalUrlCount}
                    </span>
                  </div>
                  <div className="w-full bg-neutral-200 rounded-full h-2.5 mb-4 overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.max(5, ((fetchedProblems.length + failedUrls.length) / parseResult.totalUrlCount) * 100)}%`,
                      }}
                    ></div>
                  </div>
                  ...
                  <div className="mb-6">
                    ...
                    <ul className="space-y-2">
                      {failedUrls.map((url, i) => (
                        <li
                          key={i}
                          className="text-xs text-rose-600 bg-rose-50 px-3 py-2 rounded-lg truncate"
                        >
                          {url}
                        </li>
                      ))}
                  ...
                  <ul className="space-y-2">
                    {fetchedProblems
                      ...
                      .map((p, i) => (
                        <li
                          key={i}
                          className="text-xs flex items-center justify-between bg-white border border-neutral-100 shadow-sm px-3 py-2 rounded-lg"
                        >
                          <span className="font-bold text-neutral-800 truncate pr-4">
                            {p.title}
                          </span>
                          <span
                            className={`px-2 py-0.5 rounded-full text-[10px] font-bold tracking-wide uppercase border shrink-0 ${
                              p.difficulty === "Easy"
