...
export default function App() {
  ...
  const [highlightedProblemId, setHighlightedProblemId] = useState<
    string | null
  >(null);
  ...
  return (
    <div
      id="dsa-app-root"
      className="min-h-screen bg-[#F4F1FA] text-[#332F3A] font-sans antialiased flex flex-col relative overflow-hidden"
    >
      ...
      <header
        id="dsa-navbar"
        role="banner"
        className="shrink-0 sticky top-4 sm:top-6 z-40 w-full px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto"
      >
        <div
          id="navbar-container"
          className="h-16 w-full mx-auto px-4 sm:px-6 flex items-center justify-between bg-white/70 backdrop-blur-xl rounded-[32px] shadow-clay-surface border-2 border-white/50 transition-all duration-300"
        >
          ...
          <SignedIn>
            <nav role="navigation">
              <div
                ...
                role="tablist"
                className="bg-[#F4F1FA]/60 p-1.5 rounded-[20px] shadow-inner hidden sm:flex gap-1 border border-white/60"
              >
                <Tooltip content="Home Dashboard" shortcut="Home">
                  <button
                    ...
                    className={`px-4 py-2 rounded-2xl text-xs font-bold transition-all duration-300 cursor-pointer flex items-center gap-1.5 whitespace-nowrap ${
                      activeMainTab === "home"
                        ? "bg-white text-indigo-700 shadow-clay-button border border-white/80"
                        : "text-[#635F69] hover:text-[#332F3A] hover:bg-white/40"
                    }`}
                  >
          ...
          <div id="navbar-controls-sec" className="flex items-center gap-2">
            ...
            <SignedOut>
              <button
                ...
                className="px-5 py-2.5 bg-gradient-to-br from-indigo-500 to-indigo-700 text-white rounded-[20px] text-sm font-bold flex items-center gap-2 active:scale-[0.95] active:shadow-clay-pressed hover:-translate-y-1 hover:shadow-clay-button-hover transition-all duration-300 cursor-pointer shadow-clay-button font-nunito"
              >
                              ...
                              {loading ? (
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    parameters:{" "}
                                    <code>
                                      {searchQuery || "Multiple Filters"}
                                    </code>
                                    . Try resetting filters or creating a new
                                    document in the collection folders.
                                  </p>
                                  <button
